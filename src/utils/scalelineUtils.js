...
export function getGridSize(scale) {
...
/** 获取当前scale下的单元格像素 */
export function getGridPixel(scale, frameCount) {
    let gridPixel = getGridSize(scale);
    let trackWidth = gridPixel * frameCount * 30;
    if (scale < 70) { // 1秒一格
...
/** 获取选中点的秒数 */
export function getSelectTime(offsetX, scale, frameStep) {
    ...
    return Math.floor((offsetX / size + (scale < 70 ? 0 : 1)) / frameStep);
}

/** 根据缩放比调整 step */
export function getStep(scale, frameStep){
    return scale > 60 ? frameStep : 10;
}

/** 获取长文本 */
export function getLongText(count, scale) {
...
/** 获取短文本 */
export function getShortText(count, step, scale) {
...
/** 时间轴画线 */
export function drawTimeLine (context, userConfigs, canvasConfigs) {
    ...
    if (focusPosition) {
        ...
        const focusS = (fStart * gridSizeS + canvasConfigs.lineWidth - start); // 选中起点坐标
        const focusW = (fCount * gridSizeS - canvasConfigs.lineWidth); // 选中宽度
    ...
    for (let value = startValueB, count = 0; value < endValue; value += gridSizeB, count++) {
        const x = offsetXB + count * gridSizeB + canvasConfigs.lineWidth; // prevent canvas 1px line blurry
        context.moveTo(x, 0);
        context.save();
    ...
    for (let value = startValueS, count = 0; value < endValue; value += gridSizeS, count++) {
        const x = offsetXS + count * gridSizeS + canvasConfigs.lineWidth; // prevent canvas 1px line blurry
        context.moveTo(x, 0);
