...
<template>
  <div class="quick-btn-group">
    <el-button v-if="supports.delete.indexOf('**') !== -1 || supports.delete.indexOf(form.type) !== -1 || supports.delete.indexOf(form.geoType) !== -1 "  type="info" size="mini" @click="removeEntity" plain>删除实体</el-button>
    <el-button v-if="supports.export.indexOf('**') !== -1 || supports.export.indexOf(form.type) !== -1 || supports.export.indexOf(form.geoType) !== -1 "  type="info" size="mini" plain>导出JSON</el-button>
  </div>
</template>
    ...
    textToAudio() {
      this.$confirm('确定根据该字幕文字生成对应音频?', '提示', {
        ...
        const options = {
          name: text,
          references: url,
          availability: this.form.startTime.replace(" ", "T")+"Z/" + this.form.endTime.replace(" ", "T") + "Z",
        }
