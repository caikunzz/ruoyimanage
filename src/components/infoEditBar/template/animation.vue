...
<template>
  ...
  >
    <el-form-item
      label="闪烁时间"
      v-if="
        supports.flicker.indexOf('**') !== -1 ||
        supports.flicker.indexOf(form.type) !== -1 ||
        supports.flicker.indexOf(form.geoType) !== -1
      "
    >
      <el-input-number
        data-unit="s"
        ...
        v-model="form.animation.flicker"
        :precision="1"
        :step="0.1"
        :max="10"
        :min="0"
    ...
    <el-form-item
      label="生长时间"
      v-if="
        supports.growthLine.indexOf('**') !== -1 ||
        supports.growthLine.indexOf(form.type) !== -1 ||
        supports.growthLine.indexOf(form.geoType) !== -1
      "
    >
      <el-input-number
        data-unit="s"
        size="mini"
        @change="changeGrowthLineDuration"
        v-model="form.animation.growthLine"
        :precision="1"
        :step="0.1"
        :max="10"
        :min="0"
    ...
    <el-form-item
      label="旋转时间"
      v-if="
        supports.rotate.indexOf('**') !== -1 ||
        supports.rotate.indexOf(form.type) !== -1 ||
        supports.rotate.indexOf(form.geoType) !== -1
      "
    >
      <el-input-number
        data-unit="s"
    ...
    <el-form-item
      label="缩放时间"
      v-if="
        supports.scale.indexOf('**') !== -1 ||
        supports.scale.indexOf(form.type) !== -1 ||
        supports.scale.indexOf(form.geoType) !== -1
      "
    >
      <el-input-number
        data-unit="s"
    ...
    <el-form-item
      v-if="
        supports.radar.indexOf('**') !== -1 ||
        supports.radar.indexOf(form.type) !== -1 ||
        supports.radar.indexOf(form.geoType) !== -1
      "
      label="雷达效果"
    >
      <el-checkbox
        size="mini"
        @change="(val) => changeAnimation(val, 'radar')"
        v-model="form.animation.radar"
        >开启</el-checkbox
      >
    </el-form-item>
    <el-form-item
      v-if="
        supports.ripple.indexOf('**') !== -1 ||
        supports.ripple.indexOf(form.type) !== -1 ||
        supports.ripple.indexOf(form.geoType) !== -1
      "
      label="空间扩散"
    >
      <el-checkbox
        size="mini"
        @change="(val) => changeAnimation(val, 'ripple')"
        v-model="form.animation.ripple"
        >开启</el-checkbox
      >
    </el-form-item>
    <el-form-item
      v-if="
        supports.trackLine.indexOf('**') !== -1 ||
        supports.trackLine.indexOf(form.type) !== -1 ||
        supports.trackLine.indexOf(form.geoType) !== -1
      "
      label="航迹线"
    >
      <el-checkbox
        size="mini"
        @change="changeAnimation"
        v-model="form.animation.trackLine"
        >开启</el-checkbox
      >
    </el-form-item>
  </el-form>
  ...
  methods: {
    ...
    changeFlickerDuration() {
      this.$cesiumHelper.updateObjById(this.form.id, {
        animation: { flicker: this.form.animation.flicker },
      });
      const info = this.$cesiumHelper.getEntityInfo(this.form.id);
      store.dispatch("updateEntitySource", info);
    },
    ...
    changeGrowthLineDuration() {
      this.$cesiumHelper.updateObjById(this.form.id, {
        animation: { growthLine: this.form.animation.growthLine },
      });
      const info = this.$cesiumHelper.getEntityInfo(this.form.id);
      store.dispatch("updateEntitySource", info);
    },
    ...
    showRadar() {
      ...
      this.$cesiumHelper.updateObjById(this.form.id, {
        animation: { radar: this.form.animation.radar },
      });
      const info = this.$cesiumHelper.getEntityInfo(this.form.id);
      store.dispatch("updateEntitySource", info);
    },
    ...
    showTrackLine() {
      this.$cesiumHelper.updateObjById(this.form.id, {
        animation: { trackLine: this.form.animation.trackLine },
      });
      const info = this.$cesiumHelper.getEntityInfo(this.form.id);
      store.dispatch("updateEntitySource", info);
    },
    showRipple() {
      ...
      this.$cesiumHelper.updateObjById(this.form.id, {
        animation: { ripple: this.form.animation.ripple },
      });
      const info = this.$cesiumHelper.getEntityInfo(this.form.id);
      store.dispatch("updateEntitySource", info);
    },
