<template>
  ...
  >
    <el-form-item
      v-if="
        supports.times.indexOf('**') !== -1 ||
        supports.times.indexOf(form.type) !== -1 ||
        supports.times.indexOf(form.geoType) !== -1
      "
      label="开始："
    >
      <el-time-picker
  ...
  methods: {
    ...
    timeChange() {
      if (this.form.type === "group") {
        const distance = dateUtils.dateDifference(
          new Date(this.entity.startTime),
          this.form.startTime
        );
      ...
      const availability =
        ...
        "/" +
        dateUtils.parseDateToString("yyyy-MM-ddTHH:mm:ssZ", this.form.endTime);
