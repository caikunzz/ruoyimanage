...
<template>
  ...
  >
    <div class="position-edit">
      <el-form-item
        v-if="
          supports.moveWay.indexOf('**') !== -1 ||
          supports.moveWay.indexOf(form.type) !== -1 ||
          supports.moveWay.indexOf(form.geoType) !== -1
        "
        label="位置："
      >
        <el-radio
          :disabled="!info.position instanceof Array"
          v-model="entity_move_way"
          @input="setPositionState"
          size="mini"
          label="drag"
          >拖拽
        </el-radio>
      ...
      <div
        v-if="
          (info.position instanceof Array &&
            supports.moveWay.indexOf('**') !== -1) ||
          supports.moveWay.indexOf(form.type) !== -1 ||
          supports.moveWay.indexOf(form.geoType) !== -1
        "
      >
        <el-form-item label="经度：">
          <el-input-number
            ...
            :disabled="entity_move_way !== 'input'"
    ...
    <div
      v-if="
        supports.height.indexOf('**') !== -1 ||
        supports.height.indexOf(form.type) !== -1 ||
        supports.height.indexOf(form.geoType) !== -1
      "
      class="height-edit"
    >
      <el-form-item label="高度：">
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      this.form.position =
        this.entity.position instanceof Array
          ? {
      ...
      if (entity.type === "video" || entity.type === "text") {
        ...
        this.form.position =
          data.position instanceof Array
            ...
            : {
                x: data.position.points[0][0],
                y: data.position.points[0][1],
                z: data.position.points[0][2],
              };
      }
    },
    /** 位置修改 */
    ...
    bindDragDom(event) {
      ...
      function stopDrag(event) {
        ...
        const coord = [
          that.form.position.x,
          that.form.position.y,
          that.form.position.z,
        ];
