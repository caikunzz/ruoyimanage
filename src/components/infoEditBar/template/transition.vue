...
-->
<template>
  <el-form  class="transition-form" label-position="right" label-width="50px" :model="form">
    <div class="transition-form-item-default">
      <div class="transition-default-radio-bar">
        <el-radio v-model="entity_camera_transition" @input="switchCameraTransition('default')" size="mini" label="default">
          默认过渡
        </el-radio>
        <el-dropdown
      ...
      <div class="transition-form-item-content" v-if="entity_camera_transition==='default'">
    ...
    <div class="transition-form-item-rotate">
      <el-radio v-model="entity_camera_transition" @input="switchCameraTransition('rotate')" size="mini" label="rotate">
        绕点旋转
      </el-radio>
      <div class="transition-form-item-content" v-if="entity_camera_transition==='rotate'">
        <el-form-item label="中心点：">
          <el-input disabled size="mini" v-model="value"></el-input>
          <el-button type="warning" icon="el-icon-edit" circle size="mini" @click="setCenter"></el-button>
        </el-form-item>
      </div>
    </div>
  </el-form>
  ...
  methods:{
    ...
    init(){
      ...
      if (this.entity.angle.look){
        ...
        this.$cesiumHelper.plotting.tagPlotting.showPlottingForData(
            {
              id: uuid.uuid(),
              name: "中心点",
              show: true,
            ...
            }).then(entity=>this.center=this.$cesiumHelper.getEntityInfo(entity))
      } else {
