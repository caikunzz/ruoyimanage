  ...
  <div>
    ...
    >
      <div class="run-form-item-draw">
        <el-form-item
          v-for="(item, index) in form.path.data.draw"
          :key="index"
          :label="
            index === 0
              ? '起点：'
              : index === form.path.data.draw.length - 1
              ? '终点：'
              : '路径：'
          "
        >
          <div style="display: flex; align-items: center">
            <el-input
              class="draw-point-input"
              v-model="form.path.data.draw[index].name"
              size="mini"
            >
              <el-button
                slot="append"
                icon="el-icon-close"
                @click="removePathPoint(index, item.id)"
                :disabled="index === 0"
              ></el-button>
            </el-input>
        ...
        <el-form-item
          :label="form.path.data.draw.length === 0 ? '起点：' : '路径：'"
        >
          <el-button
            @click="addRunPath"
            class="add-point"
    ...
    >
      <el-form class="run-form" label-position="right" label-width="100px">
        <el-table size="mini" :data="tableData" :border="true">
          <el-table-column label="运动时间" width="180">
            <template slot-scope="scope">
              <el-time-picker
                ...
                :picker-options="{
                  selectableRange: `${
                    scope.row.date[1] ? scope.row.date[1] + ':00' : '00:00:00'
                  }-23:59:00,`,
                }"
              ></el-time-picker>
            </template>
          </el-table-column>
          <el-table-column label="起始点" width="100">
            <template slot-scope="scope">
              <el-select
                v-model="scope.row.start"
                size="mini"
                @change="selectStartPoint(scope.$index)"
              >
                <el-option
          ...
          <el-table-column label="结束点" width="100">
            <template slot-scope="scope">
              <el-select
                v-model="scope.row.end"
                size="mini"
                @change="selectEndPoint(scope.$index)"
              >
                <el-option
                  v-for="item in pointOptions"
                  :key="item.id"
          ...
          <el-table-column label="运动状态" width="78">
            <template slot-scope="scope">
              <el-checkbox
                @change="changRunState(scope.$index)"
                size="mini"
          ...
          <el-table-column label="添加" width="55">
            <template slot-scope="scope">
              <el-button
                style="margin-left: 5px"
                :disabled="isDisabled"
                @click="addRunPathPoints(scope.row)"
                class="add-point"
                size="mini"
                icon="el-icon-plus"
                circle
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      // 如果有路径  则回显路径
      if (
        this.$cesiumHelper.plotting.routePlotting.showEntityPath(this.form.id)
      )
        this.showPathEdit();
      //切换编辑对象清除运动路线
      this.cleanPathEdit();
    ...
    cleanPathConfig() {
      ...
      entity.position = plottingUtils.latitudeAndLongitudeToDegrees(
    ...
    setRunTime(index) {
      if (index === 0 && this.form.path.data.draw.length < 2)
        return this.$message.info("1个路径点不支持设置时间！");
    ...
    controlAddButton() {
      if (
        this.tableData[this.tableData.length - 1].end ===
        this.editPoint.length - 1
      ) {
    ...
    addRunPath() {
      this.$cesiumHelper.plotting.tagPlotting
        ...
        .then(async (res) => {
          ...
          this.temp.pathLine.forEach((item) =>
            this.$cesiumHelper.deleteObjById(item)
          );
          this.temp.pathLine = [];
          this.form.path.data.draw.push({
            id: data.id,
            name: data.name,
            point: data.position,
          });
          ...
          const line =
            this.$cesiumHelper.plotting.routePlotting.createRoutePlotTest(
              linePath
            );
    ...
    removePathPoint(index, id) {
      ...
      line = this.$cesiumHelper.plotting.routePlotting.createRoutePlotTest(
        this.form.path.data.draw.map((item) => item.point)
      );
    ...
    showPathEdit() {
      ...
      const line =
        this.$cesiumHelper.plotting.routePlotting.createRoutePlotTest(
          entity.memory.path.data.draw.map((item) => item.point)
        );
      this.temp.pathLine.push(line.id);
    },
    /** 清除路径点编辑记录 */
    cleanPathEdit() {
      if (this.form.path) {
        this.form.path.data.draw.forEach((item) =>
    ...
    submitToDatabase() {
      ...
      const options = {
        path: this.pathLine,
        type: "draw",
        editPoint: this.editPoint.map((point) =>
          plottingUtils.latitudeAndLongitudeToDegrees(point)
        ),
      ...
      // })
      const path =
        this.$cesiumHelper.plotting.routePlotting.handlePathData(options);
      this.$cesiumHelper.plotting.routePlotting.bindRunPath(
        this.$cesiumHelper.getEntityById(this.form.id),
        path
      );
