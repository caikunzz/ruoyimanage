<template>
  <!-- 轨道区域 -->
  <div class="operate-track-list" ref="operateTrackListRef" @contextmenu="(event)=>event.preventDefault()" :style="{paddingLeft: offsetLine.left+'px'}">
    <div class="operate_track_item"
         :id="item.id"
      ...
      <vue-draggable-resizable
          ...
          @dragstop="(x,y) => {elementDrag(x,y,item)}"
          @resizestop="(x,y,w) => {elementResize(x,y,w,item)}"
          @mouseup.native.right="(event)=>handleContextmenu(event, item)"
      >
        <div style="width: 100%; height: 100%; white-space: nowrap;text-overflow: ellipsis;overflow: hidden;" @dblclick="selectEntity(item)">
          <div class="track-item-title"><span>{{ item.name }}</span></div>
        </div>
      </vue-draggable-resizable>
    </div>
    <!--  摄像头为同一行渲染  单独分开  -->
    <div id="camera-track" class="operate_track_item" :class="activeTrack && activeTrack.type === 'camera' ? 'is-active' : ''" style="height:17px" v-if="trackList.cameraList.length>0">
      <vue-draggable-resizable
          ref="dragBox"
          ...
          axis="x"
          :class="item.class"
          :style="{backgroundColor: activateEntity?.id===item.id ? '#00D7C6' : item.color}"
          :resizable="false"
          @mouseup.native.right="(event)=>handleContextmenu(event, item)"
          @activated="activatedTrack(item)"
          @deactivated="deactivatedTrack"
          @dragstop="(x,) => {cameraDrag(x,item)}"
      >
        <div style="width: 100%; height: 100%; white-space: nowrap;text-overflow: ellipsis;overflow: hidden;" @dblclick="selectEntity(item)">
...
export default {
  ...
  computed:{
    ...
    /** 实体资源 */
    entities() {
      return store.getters.entities.filter(p=>p.type!=="group" && p.type !== "root").map(item => {
        return {
          id:item.id,
          show: item.show,
          name: item.name,
  ...
  methods: {
    ...
    getDateStringFromOffsetX(x) {
      ...
      const timeNum = parseInt(timeArr[0]) * 60 * 60 + parseInt(timeArr[1]) * 60 + parseInt(timeArr[2]);
    ...
    elementDrag(x, y, item) {
      ...
      const iso8601 = times[0].replace(" ", "T") + "Z/" + times[1].replace(" ", "T") + "Z"
    ...
    render(itemList) {
      setTimeout(()=>{
        ...
        for (let i = 0; i < itemList.length; i++) {
          ...
          const startTimeSecond = transformSecond(transformTimeString(o.startTime, this.timeline.startTime))
          const endTimeSecond = transformSecond(transformTimeString(o.endTime, this.timeline.startTime))
          if (o.type === "camera") {
        ...
        this.trackList.entityList = elements.sort((a,b)=>a.groupId - b.groupId)
        this.trackList.cameraList = cameras

        const windowWidth = this.$refs.operateTrackListRef.style.width
        this.$refs.operateTrackListRef.style.width = windowWidth > maxw ? windowWidth : maxw + 100 + "px"
        this.resetTimeLine()
      }, 0)
    },
    ...
    searchTrackOffsetTop(item){
      const track = item.type === "camera" ? document.getElementById("camera-track") : document.getElementById(item.id)
