        ...
        <el-tab-pane label="地名" name="place">
          <div class="top-search-bar">
            <el-autocomplete
                ...
                :fetch-suggestions="(queryString, cb)=>{querySearchAsync(queryString, cb, 'place')}"
          ...
          <div class="tree-bar">
            <el-tree :data="placeList"
                     show-checkbox
                     node-key="id"
                     check-strictly
                     props="{label: 'label',children: 'children'}"
                     :default-checked-keys="placeList.filter(p=>p.show).map(item=>item.id)"
        ...
        <el-tab-pane label="河流" name="river">
          <div class="top-search-bar">
            <el-autocomplete
                v-model="searchName"
                popper-class="source-autocomplete-popper"
                :fetch-suggestions="(queryString, cb)=>{querySearchAsync(queryString, cb, 'river')}"
                suffix-icon="el-icon-search"
                placeholder="请输入内容"
                :trigger-on-focus="false"
                size="mini"
          ...
          <div class="tree-bar">
            <el-tree :data="riverList"
                     show-checkbox
                     node-key="id"
                     check-strictly
                     :props="{label: 'name', children: 'children'}"
                     :default-checked-keys="riverList.filter(p=>p.show).map(item=>item.id)"
                     @check="handleCheckClick"
                     :indent="8">
            <span class="custom-tree-node" slot-scope="{ node, }">
              {{ node.label }}
        ...
        <el-tab-pane label="道路" name="road">
          <div class="top-search-bar">
            <el-autocomplete
                v-model="searchName"
                popper-class="source-autocomplete-popper"
                :fetch-suggestions="(queryString, cb)=>{querySearchAsync(queryString, cb, 'road')}"
                suffix-icon="el-icon-search"
                placeholder="请输入内容"
                :trigger-on-focus="false"
                size="mini"
          ...
          <div class="tree-bar">
            <el-tree :data="roadList"
                     show-checkbox
                     node-key="id"
                     check-strictly
                     :props="{label: 'name',children: 'children'}"
                     :default-checked-keys="roadList.filter(p=>p.show).map(item=>item.id)"
                     @check="handleCheckClick"
                     :indent="8">
            <span class="custom-tree-node" slot-scope="{ node, }">
              {{ node.label }}
        ...
        <el-tab-pane label="边界" name="boundary">
          <div class="top-search-bar">
            <el-autocomplete
                v-model="searchName"
                popper-class="source-autocomplete-popper"
                :fetch-suggestions="(queryString, cb)=>{querySearchAsync(queryString, cb, 'boundary')}"
                suffix-icon="el-icon-search"
                placeholder="请输入内容"
                :trigger-on-focus="false"
                size="mini"
          ...
          <div class="tree-bar">
            <el-tree :data="boundaryList"
                     show-checkbox
                     node-key="id"
                     check-strictly
                     :default-checked-keys="boundaryList.filter(p=>p.show).map(item=>item.id)"
                     :props="{label: 'name',children: 'children'}"
                     @check="handleCheckClick"
                     :indent="8">
            <span class="custom-tree-node" slot-scope="{ node, }">
  ...
  methods: {
    ...
    async addPlace(id) {
      resourceApi.queryCityById(id).then(res=>{
        ...
        this.$cesiumHelper.plotting.placePlotting.showPlottingForData(options).then(entity => {
    ...
    async addRiver(id) {
      ...
      this.$cesiumHelper.plotting.riverPlotting.showPlottingForData(options).then(entity => {
        const info = this.$cesiumHelper.getEntityInfo(entity);
        store.dispatch("addEntitySource", info);
      }, err => this.$message.warning(err));
    },
    ...
    async addRoad(id) {
      ...
      this.$cesiumHelper.plotting.roadPlotting.showPlottingForData(options).then(entity => {
        const info = this.$cesiumHelper.getEntityInfo(entity);
        store.dispatch("addEntitySource", info);
      }, err => {
    ...
    async addBoundary(id) {
      ...
      this.$cesiumHelper.plotting.boundaryPlotting.showPlottingForData(options).then(entity => {
        const info = this.$cesiumHelper.getEntityInfo(entity);
        store.dispatch("addEntitySource", info);
      }, err => {
        this.$message.warning(err)
