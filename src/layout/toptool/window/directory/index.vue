<template>
  ...
  <floating-bar
    ...
    <template slot="default">
      ...
      <!-- 右键菜单 -->
      <contextMenu ref="directoryContextMenuRef" class="context-bar"></contextMenu>
    </template>
  </floating-bar>
</template>
...
export default {
  ...
  computed: {
    directory() {
      const list = this.$store.getters.entities.map(item => {
        return {id: item.id, name: item.name, type: item.type, groupId: item.groupId}
      })
  ...
  methods: {
    ...
    allowNodeDrop(_, dropNode, type){
      return (dropNode.data.groupId !==undefined || type === "inner")
