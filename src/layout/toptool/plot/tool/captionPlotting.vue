<template>
  ...
  <el-dialog
    ...
    <div class="caption-out-header">
      <div class="btn-group">
        <el-button size="mini" plain type="warning" @click="openInnerDialog">添加</el-button>
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <el-button
              type="text"
              size="mini"
              @click="editInnerDialog(scope.row)"
          >编辑</el-button>
          <el-button
              size="mini"
              type="text"
              @click="delCaption(scope.row)"
          >删除</el-button>
        </template>
      </el-table-column>
    </el-table>
      ...
      <el-form size="mini" :model="form" label-position="right">
        <el-form-item label="字幕时间：" label-width="90px">
          <div class="times-set">
            ...
            <el-button @click="computeAudioTime" type="primary" :loading="autoLoading" :disabled="form.label.length===0" size="mini">自动</el-button>
          </div>
        </el-form-item>
        <el-form-item label="字幕内容：" label-width="90px">
          <el-input size="mini" style="width: 100%" type="textarea" :rows="2" v-model="form.label"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="closeInnerDialog" size="mini">取 消</el-button>
        <el-button type="primary" :loading="confirmLoading" @click="submitCaption" size="mini">添 加</el-button>
      </div>
    </el-dialog>
    <div slot="footer" class="dialog-footer">
      <el-button size="mini" @click="closeOutDialog">取 消</el-button>
      <el-button type="primary"  @click="closeOutDialog" size="mini">确 定</el-button>
    </div>
  </el-dialog>
</template>
...
export default {
  ...
  computed:{
    tableData() {
      return store.getters.entities.filter(p=>p.name  && p.name.indexOf(this.search) !== -1 && p.type==="captions").map(item => {
        return {id: item.id, name: item.name, label: item.label, references: item.references, startTime: item.startTime, endTime: item.endTime}
  ...
  methods: {
    ...
    openInnerDialog(){
      const startTime = new Date(store.state.cesium.currentTime.replace("T", " ").replace("Z", ""))
      this.form.id = null
    ...
    closeInnerDialog(){
      this.form.label = ""
      this.form.times = [new Date(2000, 0, 1, 0, 0, 0), new Date(2000, 0, 1, 0, 0, 5)]
      this.innerDialogFormVisible = false
    },
    /** 添加字幕 */
    submitCaption() {
      const times = dateUtils.parseDateToString("yyyy-MM-ddTHH:mm:ssZ", this.form.times[0]) + "/" + dateUtils.parseDateToString("yyyy-MM-ddTHH:mm:ssZ", this.form.times[1])
      const options = {
