    ...
    <el-collapse accordion v-model="activeName">
      <el-collapse-item title="文本转换">
        <div class="audio-group-body">
          <div class="audio-item text-to-audio" @click="dialogFormVisible=true">
            <img draggable="false" class="audio-icon" src="../../../../assets/images/audio/text_to_audio.png" />
            <span class="text" style="margin-top: auto">文本转语音</span>
          </div>
        </div>
      </el-collapse-item>
      <el-collapse-item v-for="(item, index) in audioList" :key="index" :title="item.name" :name="index">
        <div class="audio-group-body">
          <div class="audio-item" v-for="(i_item, i_index) in item.value" :key="i_index">
            <img class="audio-icon" @dragend="addAudio(i_item)" src="../../../../assets/images/audio/effect_0.gif" />
      ...
      <el-collapse-item title="导入文件" v-loading="customAudioLoading">
        <div class="input-wrapper">
          ...
          <el-button size="mini" type="primary" @click="getCustomAudioList(1)" icon="el-icon-search">搜索</el-button>
          <el-upload
              size="mini"
              class="upload-demo"
        ...
        <div class="audio-group-body">
          <div class="audio-item audio-item-custom" v-for="(i_item, i_index) in customAudio" :key="i_index">
            <img
    ...
    <el-dialog
      ...
      <el-form :model="textToAudioForm" label-position="right">
        <el-form-item label="文本：" label-width="90px">
          <el-input size="mini" style="width: 100%" type="textarea" :rows="2" v-model="textToAudioForm.text"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="closeDialog" size="mini">取 消</el-button>
        <el-button type="primary" @click="textToAudio" size="mini">添 加</el-button>
      </div>
    </el-dialog>
  </div>
</template>
  ...
  methods: {
    ...
    getCustomAudioList(val){
      ...
      fileNodeApi.list(this.queryParams).then(res=>{
        ...
        this.customAudio = res.rows.map(item=>{return {id:item.id, text: item.fileName, url: item.fileUrl}})
    ...
    async addAudio(options) {
      const entity = await this.$cesiumHelper.audio.addAudio({name: options.text, references: options.url})
