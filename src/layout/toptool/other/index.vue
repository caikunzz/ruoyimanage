          ...
          <el-form-item label="链接">
            <div class="link-rows">
              ...
              <el-button
                ...
                @click="copyLink(cooperateForm.link)"
                type="success"
                >拷贝链接</el-button
              >
            </div>
...
export default {
  ...
  methods: {
    ...
    splitScreen() {
      this.split
        ? this.$cesiumHelper.endSplitScreen()
        : this.$cesiumHelper.startSplitScreen();
    ...
    openOutCooperate() {
      if (this.$store.getters.mode === "coordination") {
        this.cooperateForm = {
          name: "Your room name",
          link: `http://${window.location.hostname}:${window.location.port}?room=${this.$route.query.room}`,
        };
    ...
    openInnerCooperate() {
      ...
      this.cooperateForm = {
        name: "Your room name",
        link: `http://${window.location.hostname}:${window.location.port}?room=${room}`,
      };
      this.cooperateInnerVisible = true;
      this.cooperateOutVisible = false;
    },
    ...
    copyLink(link) {
      navigator.clipboard
        .writeText(link)
        .then(() => this.$message.success("链接已复制到剪切板"))
        .catch(() => this.$message.error("链接复制失败，请重试"));
    },
    /** 控制设置视频弹框 */
