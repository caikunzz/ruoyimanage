...
class CesiumHelper {
    ...
    constructor(setting) {
        ...
        Cesium.Ion.defaultAccessToken = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJqdGkiOiIyZjI0Njg3Mi02N2UyLTQ3ZGUtODI5MC0wNWEzMGIxYTNmM2QiLCJpZCI6MTIyNzM0LCJpYXQiOjE2OTI5NDM5MDZ9.VJXePGUbUwfN33NHCKP-IQIA16JieiJ7QN_7IZJqxyE";
        this.viewer = new Cesium.Viewer(setting.container, {
            ...
            contextOptions: {       //将把cesium退回到WebGL1 否则流光线材质报错
                requestWebgl1: true,
            },
        })
        ...
        this.viewer.scene.debugShowFramesPerSecond = setting.showFramesPerSecond || false; // 显示帧率
        this.viewer.canvas.willReadFrequently = true;
        this.viewer.scene.globe.depthTestAgainstTerrain = setting.depthTestAgainstTerrain || false; //禁用深度写入
        this.viewer.cesiumWidget.creditContainer.style.display = "none"; //隐藏logo
        this.viewer.animation.container.style.visibility = setting.showClock ? 'visible' : 'hidden' //隐藏时钟
        this.viewer.timeline.container.style.display = setting.showTimeline ? 'block' : 'none'; //隐藏时间线
        ...
        if (setting.imageryLayers) {
            this.viewer.imageryLayers.addImageryProvider(new Cesium.UrlTemplateImageryProvider({
                ...
                fileExtension: setting.imageryLayers.split(".")[setting.imageryLayers.split(".").length - 1]
            }))
            console.log("已加载：【本地影像】");
        }
        // 本地地形加载
        if (setting.terrainProvider) {
            Cesium.CesiumTerrainProvider.fromUrl(setting.terrainProvider).then(terrainProvider => {
                this.viewer.terrainProvider = terrainProvider
                console.log("已加载：【本地地形】");
            }).catch(err => console.log(err))
        }
        // 是否支持图像渲染像素化处理
        if (Cesium.FeatureDetection.supportsImageRenderingPixelated()) this.viewer.resolutionScale = window.devicePixelRatio
        // 开启抗锯齿
        this.viewer.scene.postProcessStages.fxaa.enabled = setting.enableFxaa || false;
    ...
    changeMapSource(url) {
        ...
        this.mapTiles = this.viewer.imageryLayers.addImageryProvider(imageryProvider);
    }
    ...
    startSplitScreen() {
        if (!this.viewerSplit) {
            ...
            if (this.setting.splitLayers) {
                this.viewerSplit.imageryLayers.addImageryProvider(new Cesium.UrlTemplateImageryProvider({
                    url: this.setting.splitLayers,
                    fileExtension: this.setting.splitLayers.split(".")[this.setting.splitLayers.split(".").length - 1]
                }))
                console.log("已加载：【分屏本地影像】");
            }
            // 本地地形加载
            if (this.setting.terrainProvider) {
                Cesium.CesiumTerrainProvider.fromUrl(this.setting.terrainProvider).then(terrainProvider => {
                    this.viewer.terrainProvider = terrainProvider
                    console.log("已加载：【本地地形】");
                }).catch(err => console.log(err))
            }
    ...
    setActiveEntity(value) {
        if (!value) {
            ...
            this.activeElement = value instanceof Object ? value : this.getEntityById(value)
        }
    }
    ...
    export(nodeId) {
        ...
        if (nodeId == null) {
            ...
            nodes = [...plottingList, ...captionList, ...audioList, ...videoList, ...cameraList, ...textList, ...groupList]
        } else {
        ...
        animation.date = dateUtils.parseDateToString("yyyy-MM-dd HH:mm:ss", new Date())
        return animation
    }
     ...
     */
    loadEntity(item) {
        item.availability = dateUtils.parseDateToString("yyyy-MM-ddTHH:mm:ssZ", new Date(item.startTime)) + "/" + dateUtils.parseDateToString("yyyy-MM-ddTHH:mm:ssZ", item.endTime)
        ...
        switch (item.type) {
            ...
            case "particle":
            case "road":
                return new Promise(resolve => resolve(this.plotting.showPlottingForData(item)))
            case "captions":
                return new Promise(resolve => resolve(this.captions.loadCaptions(item)))
            case "audio":
                return new Promise(resolve => resolve(this.audio.loadAudioOption(item)))
            case "video":
                return new Promise(resolve => resolve(this.video.loadVideoOption(item)))
            case "text":
                return new Promise(resolve => resolve(this.text.loadTextOption(item)))
            case "camera":
                return new Promise(resolve => resolve(this.camera.loadCameraOption(item)))
            case "root":
            case "group":
                return new Promise(resolve => resolve(this.group.loadGroupForData(item)))
            default:
    ...
    loadSourceFromJson(animation) {
        ...
        return new Promise((resolve, reject) => {
            Promise.all(promiseList).then(list => {
                /** 更新资源 */
                this.setCurrentTime(dateUtils.julianDateToIso8601(this.viewer.clock.currentTime))
                /** 标记要改 */
                resolve(list)
            }).catch(err => {
                console.log(err);
    ...
    _getResourceAvailability() {
        const list = Array.from(this.viewer.resource.values()).filter(p => p.type !== "group" && p.type !== "root")
        if (list.length === 0) return null
        const start = list.reduce((pre, cur) => new Date(pre.startTime).getTime() > new Date(cur.startTime).getTime() ? cur : pre)
        const end = list.reduce((pre, cur) => new Date(pre.endTime).getTime() > new Date(cur.endTime).getTime() ? pre : cur)
        return start.startTime.replace(" ", "T") + "Z/" + end.endTime.replace(" ", "T") + "Z"
    }
    ...
    _moveEvent() {
        ...
        handler.setInputAction((movement) => {
            if (Cesium.defined(pickedObject) && pickedObject.id instanceof Cesium.Entity) {
                ...
                if (move && (pickedObject.id.model || pickedObject.id.billboard || pickedObject.id.label) && !pickedObject.id.polyline && !pickedObject.id.polygon) {
     ...
     */
    jumpToEntity(info) {
        if (info.type === "image" || info.type === "model" || info.type === "tag" || info.type === "particle" || info.type === "place") {
            ...
            if (entity.polyline) positions = entity.polyline.positions.getValue(Cesium.JulianDate.now());
            // 处理面实体：同时获取 polygon 的顶点
            if (entity.polygon) positions = entity.polygon.hierarchy.getValue(Cesium.JulianDate.now()).positions;
            this.flyToEntity(positions)
        } else if (info.type === "river" || info.type === "boundary" || info.type === "road") {
            const Container = []
            ...
            entitys.children.forEach((item) => {
                ...
                if (!line) {
                    line = item.polygon.hierarchy.getValue(Cesium.JulianDate.now()).positions;
                }
                Container.push(line)
            })
    ...
    flyToEntity(positions) {
        if (!(typeof positions[0] === 'object')) {
            ...
            // 计算包围盒的中心点和半径
            const center = Cesium.Cartesian3.midpoint(boundingBox.minimum, boundingBox.maximum, new Cesium.Cartesian3());
            const radius = Cesium.Cartesian3.distance(boundingBox.minimum, boundingBox.maximum) / 2;
            ...
            this.viewer.camera.flyToBoundingSphere(new Cesium.BoundingSphere(center, radius), {
                ...
                offset: new Cesium.HeadingPitchRange(0.0, -Cesium.Math.PI_OVER_TWO, radius * 2.5) // 调整相机偏移量
            });
        }
    }
    ...
    _clickEvent() {
        ...
        handler.setInputAction(click => {
            ...
            if (Cesium.defined(pickedObject)) { // 触发实体激活状态改变事件
                this.setActiveEntity(pickedObject.id)
                Array.from(this.activateEntityEventMap.values()).forEach(func => func(click, pickedObject.id))
            }
    ...
    _contextmenuEvent() {
        ...
        document.addEventListener("contextmenu", (event) => {
            ...
            if (target.className !== "" && (target.className === "cesium-helper-text" || target.className === "cesium-helper-video")) {
                event.preventDefault()
                const entity = this.getEntityById(target.dataset.id)
                if (entity != null) this.contextmenuEvent({position: {x: event.x, y: event.y}}, entity)
            }
        })
    }
    ...
    windowLocationToLatitudeAndLongitude(event) {
        return new Promise(resolve => {
            ...
            // 屏幕坐标转换为世界坐标
            const worldPosition = this.viewer.scene.globe.pick(this.viewer.camera.getPickRay(windowPosition), this.viewer.scene);
            if (Cesium.defined(worldPosition)) {
                // 获取经纬度和高度
                const cartographic = this.viewer.scene.globe.ellipsoid.cartesianToCartographic(worldPosition);
