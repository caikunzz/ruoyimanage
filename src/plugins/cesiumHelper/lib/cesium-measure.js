      ...
      drawLineMeasureGraphics: function (options = {}) {
        if (this._viewer && options) {
          ...
          _handlers.setInputAction(function (movement) {
            ...
            _handlers.destroy();
            _handlers = null;
            _lineEntity.polyline.positions =
              _lineEntity.polyline.positions.getValue(); //取消CallbackProperty
          ...
          _lineEntity.polyline = {
            ...
            material:
              Cesium.Color.fromCssColorString(options.material) ||
      ...
      drawAreaMeasureGraphics: function (options = {}) {
        if (this._viewer && options) {
          ...
          _handler.setInputAction(function (movement) {
            window.toolTip =
              "点击鼠标左键继续绘制，双击鼠标左键结束绘制（不少于3个标绘点）。";
            var cartesian = $this.getCatesian3FromPX(movement.position);
            if (cartesian && cartesian.x) {
              if (positions.length == 0) {
          ...
          _handler.setInputAction(function (movement) {
            ...
            _polygonEntity.polyline.positions =
              _polygonEntity.polyline.positions.getValue(); //取消CallbackProperty
            positions.push(positions[0]);
        ...
        if (this._viewer && options) {
          ...
          _handler.setInputAction(function (movement) {
            ...
            if (_positions.length >= 1) {
              //将所有CallbackProperty回调取消
              const linePoitions1 =
                _trianglesEntity.polyline.positions.getValue();
              const linePoitions2 =
                _tempLineEntity.polyline.positions.getValue();
              const linePoitions3 =
                _tempLineEntity2.polyline.positions.getValue();
              _trianglesEntity.polyline.positions = linePoitions1;
              _trianglesEntity.position = linePoitions1[0];
              _trianglesEntity.label.text =
                "直线:" +
                $this.getPositionDistance(
                  $this.transformCartesianArrayToWGS84Array(linePoitions1)
                ) +
                " m";
              _tempLineEntity.polyline.positions = linePoitions2;
              _tempLineEntity.positions = linePoitions2[1];
              _tempLineEntity.label.text =
                "高度:" +
                _getHeading(linePoitions2[0], linePoitions2[1]) +
                " m";
              _tempLineEntity2.polyline.positions = linePoitions3;
              _tempLineEntity2.position = linePoitions3[1];
              _tempLineEntity2.label.text =
                "水平距离:" +
                $this.getPositionDistance(
                  $this.transformCartesianArrayToWGS84Array(linePoitions3)
                ) +
                " m";
              toolTips(window.toolTip, null, false);
            }
          ...
          _tempLineEntity.label = {
            text: new Cesium.CallbackProperty(function () {
              return (
                "高度:" + _getHeading(_tempPoints[0], _tempPoints[1]) + " m"
              );
            }, false),
            show: true,
            showBackground: true,
