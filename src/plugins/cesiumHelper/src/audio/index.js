...
import {julianDateFromIso8601, julianDateToIso8602Times} from "../common/dateUtils";
...
class Audio{
    ...
    constructor(viewer, container, supper) {
        ...
        supper.player.addEventListener("play", "playAudio", this.playAudio.bind(this))
        supper.player.addEventListener("pause", "pauseAudio", this.pauseAudio.bind(this))
        supper.player.addEventListener("change", "updateAudio", this.updateAudioState.bind(this))
        supper.player.addEventListener("speed_change", "speed_change_audio_callback", this._speedChangeCallback.bind(this))
    }
    /**
     * @description: 添加音频
     *
    ...
    addAudio(options){
        return new Promise((resolve, reject) => {
            try {
                ...
                const startTime = options.availability ? julianDateFromIso8601(options.availability.split("/")[0]) : this.viewer.clock.currentTime
                this._createAudioContainer(options.references).then(
                    audio => {
                        const distance = Math.round(audio.duration) && Math.round(audio.duration)!==Infinity ? Math.round(audio.duration) : 5
                        const temp = {
                            id: id,
                            ...
                            availability: options.availability || julianDateToIso8602Times(startTime, distance),
    ...
    updateAudioState(currentTime){
      ...
      for (const [key, value] of this.audioMap) {
        ...
        if (currentTime >= startTime && currentTime < endTime && frameTran){
          ...
          value.html.currentTime = Math.abs(currentTime.getTime() - startTime.getTime()) / 1000
     ...
     * */
    _getDefaultName() {
        return "音频" + ("000" + (Array.from(this.audioMap.keys()).length+1)).substr(-3);
    }
