...
class Text{
    ...
    constructor(viewer, cesiumContainer, supper) {
        ...
        /** 设置 系统时间回调 */
        supper.player.addEventListener("change", "updateText", this.updateText.bind(this))
    }
    ...
    addText(options){
        ...
        return new Promise(resolve => {
            ...
            options.position = options.position || [this.supper.cesiumContainer.clientWidth / 2 - 29.5, this.supper.cesiumContainer.clientHeight / 2 - 10.5]
            options.name = options.name || this._getDefaultName()
            options.show = options.show !== undefined && options.show != null ? options.show : true
            options.material = {
                text: {
            ...
            this._createTextDom(options).then(dom=>{
                const entity = {
                    ...
                    availability: options.availability || julianDateToIso8602Times(this.viewer.clock.currentTime),
    ...
    editText(id, options){
        ...
        if ("material" in options){
            ...
            if (material.text){
                ...
                if (text.outline){
                    entity.html.style.textShadow = "0 0 3px "+options.material.text.outline+",0 0 3px "+options.material.text.outline+",0 0 3px "+options.material.text.outline+",0 0 3px "+options.material.text.outline
    ...
    copy(entity) {
        return new Promise((resolve, reject) => {
            ...
            this.loadTextOption(data).then(ent => resolve(ent)).catch(err=>reject(err))
        })
    }
    ...
    _createTextDom(options){
        return new Promise(resolve => {
            ...
            dom.style.fontStyle = options?.material?.text?.italic ? 'italic' : 'unset'
            options?.material?.text?.family && (dom.style.fontFamily = options.material.text.family)
            options?.material?.text?.size && (dom.style.fontSize = options.material.text.size + 'px')
            options?.material?.text?.color && (dom.style.color = options.material.text.color)
            options?.material?.fill?.color && (dom.style.backgroundColor = options.material.fill.color)
            options?.material?.text?.outline && (dom.style.textShadow = "0 0 2px "+options.material.text.outline+",0 0 2px "+options.material.text.outline+",0 0 2px "+options.material.text.outline+",0 0 2px "+options.material.text.outline)
