...
import {
    calculatePincerArrowPoint, lockingMap,
...
class PincerArrowClass{
    ...
    createPincerArrow(options){
        return new Promise((resolve, reject) => {
            ...
            handler.setInputAction((event) => {
                ...
                showLinePoints.push(cartesian)
                if (!cartesian || anchorpoints.length > 3) return; //设置钳击箭头控制点
            ...
            handler.setInputAction((move) => {
                ...
                if (anchorpoints.length >= 2) {
                    if (!Cesium.defined(pincerArrow)) {
                        anchorpoints.push(cartesian);
                        pincerArrow = this.viewer.entities.add({
                            id: id,
                            name: 'PincerArrow',
                            type: "situation",
                            ...
                            polyline: {
                                positions: new Cesium.CallbackProperty(function () {
                                    const positions = calculatePincerArrowPoint(anchorpoints); // 根据多边形顶点计算多边形形状
                                    //positions会出现null
            ...
            handler.setInputAction((event) => {
                ...
                updateEntityToStaticProperties(pincerArrow)
                if (options?.show instanceof Array){
                    pincerArrow.polyline.distanceDisplayCondition = new Cesium.DistanceDisplayCondition(...options.show)
                    pincerArrow.polygon.distanceDisplayCondition = new Cesium.DistanceDisplayCondition(...options.show)
                }
            ...
            handler.setInputAction(() => {
                if (anchorpoints.length === 1) return; //确保数组不为空
                    anchorpoints.pop();
            }, Cesium.ScreenSpaceEventType.RIGHT_DOWN);
            const cancel = (e) => {
                if (e.key === 'Escape') {
                    this.viewer.entities.remove(pincerArrow)
                    handler.destroy();
     ...
     *
     */
    editPincerArrow(viewer, entity, handler, collection){
        ...
        let currentPickPolygon = undefined; //当前选择的要移动的钳击箭头
        // 左键摁下事件
        handler.setInputAction((event) => {
            boolDown = true;
    ...
    showPincerArrow(options){
        return new Promise((resolve, reject) => {
            ...
            if (existingEntity) {
                ...
                const degreesArrayHeight = calculatePincerArrowPoint(degreesPositions).map(item=>{
     ...
     */
    editPincerArrowData(source,options){
        //编辑状态实体
        ...
        this.viewer.resource.set(source.id, this.getPincerArrowJson(source))
    }
    /**
     * 导出态势箭头 Json 数据
     *
...
export default PincerArrowClass
