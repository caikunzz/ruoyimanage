...
class AttackArrowClass {
    ...
    return new Promise((resolve, reject) => {
      ...
      handler.setInputAction((event) => {
        ...
        if (options?.show instanceof Array) {
          attackArrow.polyline.distanceDisplayCondition =
            new Cesium.DistanceDisplayCondition(...options.show);
          attackArrow.polygon.distanceDisplayCondition =
            new Cesium.DistanceDisplayCondition(...options.show);
        }
        handler.destroy();
  ...
  editAttackArrow(viewer, entity, handler, collection) {
    ...
    handler.setInputAction(() => {
      ...
      if (entity.polygon.material instanceof Cesium.ImageMaterialProperty) {
        const color =
          entity.polygon.material["image"]?.getValue()?.dataset.color;
  ...
  showAttackArrow(options) {
    return new Promise((resolve, reject) => {
      ...
      if (existingEntity) {
        ...
        const degreesArrayHeight = getAttackArrowPoints(degreesPositions).map(
          (item) => {
        ...
        if ("material" in options) {
          if (options.material?.fill?.fade) {
            entity.polygon.material = this.tailFade(
              entity,
              options.material.fill.color
            );
          }
      ...
      if (material.fill) {
        ...
        if (fill["fade"] !== undefined && fill["fade"] !== null) {
          ...
          if (source.polygon.material instanceof Cesium.ImageMaterialProperty) {
            const canvasColor =
              source.polygon.material["image"]?.getValue()?.dataset.color;
            source.polygon.material =
              fade || Cesium.Color.fromCssColorString(canvasColor);
            showEditEntity.polygon.material =
              fade || Cesium.Color.fromCssColorString(canvasColor);
          } else {
            source.polygon.material = this.tailFade(
              source,
              colorFormat(source.polygon.material["color"]?.getValue())
            );
            showEditEntity.polygon.material = this.tailFade(
              source,
              colorFormat(source.polygon.material["color"]?.getValue())
            );
          }
  ...
  _growAnimation(entity, duration) {
    ...
    const positions = [center, ...degreesList.slice(2, degreesList.length)].map(
      (item) => plottingUtils.degreesToLatitudeAndLongitude(item)
    ); // 经纬度
    const paths = bezierSpline(lineString(positions))
  ...
  tailFade(entity, color) {
    const degreesList = entity["PottingPoint"]; // 笛卡尔点
    // 取箭头尾部两节点 计算 尾部中点
    const A = Cesium.Cartesian3.lerp(
      degreesList[0],
      degreesList[1],
      0.5,
      new Cesium.Cartesian3()
