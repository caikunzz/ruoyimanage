...
import {
    computeSwallowtailArrow, lockingMap,
        ...
        return new Promise((resolve, reject) => {
            ...
            handler.setInputAction((event) => {
                ...
                if (options?.show instanceof Array){
                    swallowtailArrow.polyline.distanceDisplayCondition = new Cesium.DistanceDisplayCondition(...options.show)
                    swallowtailArrow.polygon.distanceDisplayCondition = new Cesium.DistanceDisplayCondition(...options.show)
                }
    ...
    showSwallowtailArrow(options) {
        return new Promise((resolve, reject) => {
            ...
            if (existingEntity) {
                ...
                const availability = dateUtils.iso8602TimesToJulianDate(options.availability);
                const degreesPositions = options.position.map(position => plottingUtils.latitudeAndLongitudeToDegrees(position));
                const degreesArrayHeight = Cesium.Cartesian3.fromDegreesArrayHeights(computeSwallowtailArrow(degreesPositions, 1)).map(item=>{
