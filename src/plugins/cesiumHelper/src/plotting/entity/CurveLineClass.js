...
class CurveLineClass {
          ...
          polyline = this.viewer.entities.add({
            ...
            polyline: {
              positions: new Cesium.CallbackProperty(function () {
                return anchorpoints.length >= 2
                  ? computeCurveLinePoints(anchorpoints, 15)
                  : null; //防止点少于两个报错
              }, false),
              width: 3,
              material:
  ...
  showCurveLine(options) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (entity) {
        ...
        const plottingPoints = options.position.map((position) =>
          plottingUtils.latitudeAndLongitudeToDegrees(position)
        );
        const degreesPositions = computeCurveLinePoints(plottingPoints).map(
          (item) => {
  ...
  getCurveLineJson(entity) {
    ...
    if (
      ...
      entity?.polyline.material instanceof Cesium.ImageMaterialProperty
    ) {
      lineType = "image";
    } else {
      lineType = "solid";
    ...
    // 获取图片的URL
    const imageUrl = entity.polyline?.material?.image?.getValue(
    ...
    if (entity.animation) {
      ...
      if (entity.animation.growthLine)
        temp["animation"]["growthLine"] = entity.animation.growthLine;
    }
  ...
  async _createLineStyle(line) {
    ...
    line.color =
      line.color instanceof Cesium.Color ? colorFormat(line.color) : line.color;
