  ...
  createCurvedPolygon(options) {
    return new Promise((resolve, reject) => {
      ...
      let constructionPoint =
        options.material.line.style === "solid" ? 100 : 10;
      // 左键点击事件
      handler.setInputAction((event) => {
        ...
        if (anchorpoints.length === 0) {
          ...
          let dynamicPositions = new Cesium.CallbackProperty(function () {
            let points = createCurvedPolygonPoints(
              anchorpoints,
              constructionPoint
            );
          ...
          polygon = this.viewer.entities.add({
            ...
            polyline: {
              positions: new Cesium.CallbackProperty(function () {
                const points = createCurvedPolygonPoints(
                  anchorpoints,
                  constructionPoint
                );
                const linePoints =
    ...
    return new Promise((resolve, reject) => {
      ...
      if (existingEntity) {
        ...
        const degreesArrayHeight = Cesium.Cartesian3.fromDegreesArrayHeights(
          createCurvedPolygonPoints(degreesPositions)
        ).map((item) => {
