...
class AssembleClass {
    ...
    createAssemble(options) {
        return new Promise((resolve, reject) => {
            ...
            let exist = this.viewer.entities.getById(id);
            if (exist) reject(exist)
            window.toolTip = '点击鼠标左键开始绘制, 按ESC取消标绘';
            const handler = new Cesium.ScreenSpaceEventHandler(this.viewer.scene.canvas);
            let anchorpoints = [];
            let assemblePolygon = undefined;
            // 左键点击事件
            handler.setInputAction((event) => {
                ...
                assemblePolygon = this.viewer.entities.add({
                    id: id,
                    ...
                    polyline: {
                        positions: new Cesium.CallbackProperty(function () {
                            const positions = computeAssemblePoints(anchorpoints); // 根据多边形顶点计算多边形形状
                            const points = Cesium.Cartesian3.fromDegreesArrayHeights(positions)
                            return points.concat(points[0]); // 将线段首尾相接形成闭合边框
                        }, false),
                        width: 2, // 设置边框线宽度
                        material: options.material.line.style === "solid" ? Cesium.Color.fromCssColorString(options.material.border.color) : new Cesium.PolylineDashMaterialProperty({
                            color: Cesium.Color.fromCssColorString(options.material.line.color), // 设置虚线的颜色
                            dashLength: 120.0, // 设置虚线段的长度
            ...
            handler.setInputAction((movement) => {
                ...
                let endCartesian = plottingUtils.getCatesian3FromPX(this.viewer, endPos);
            ...
            handler.setInputAction(() => {
                ...
                updateEntityToStaticProperties(assemblePolygon)
                if (options?.show instanceof Array){
                    assemblePolygon.polyline.distanceDisplayCondition = new Cesium.DistanceDisplayCondition(...options.show)
                    assemblePolygon.polygon.distanceDisplayCondition = new Cesium.DistanceDisplayCondition(...options.show)
                }
                handler.destroy();
                document.removeEventListener('keydown', cancel)
                document.removeEventListener("keydown",enterEnd)
                toolTips(window.toolTip, null, false);
                resolve(assemblePolygon)
            }, Cesium.ScreenSpaceEventType.LEFT_DOUBLE_CLICK);
            const cancel = (e) => {
                if (e.key === 'Escape') {
                    this.viewer.entities.remove(assemblePolygon)
                    handler.destroy();
                    toolTips(window.toolTip, null, false);
                    document.removeEventListener('keydown', cancel)
                    document.removeEventListener("keydown",enterEnd)
     ...
     *
     */
    editAssemble(viewer, entity, handler, collection) {
        ...
        let lineCallback = new Cesium.CallbackProperty(() => {
            let positions = computeAssemblePoints(updatePos);
            let points = Cesium.Cartesian3.fromDegreesArrayHeights(positions)
        ...
        if (editItem) {
            ...
            // newAssembleLine.material = Cesium.Color.WHITE;
        ...
        handler.setInputAction(() => {
            if(entity.height){
                ...
                entity.polyline.positions =  points.concat(points[0])
    ...
    showAssemble(options) {
        return new Promise((resolve, reject) => {
            ...
            const degreesArrayHeight = Cesium.Cartesian3.fromDegreesArrayHeights(computeAssemblePoints(degreesPositions)).map(item=>{
            ...
            if (existingEntity) {
                ...
                let entity = this.viewer.entities.add({
                    ...
                    // PottingPoint: options.memory.editPoint?.map(point => plottingUtils.latitudeAndLongitudeToDegrees(point)),
    ...
    /**
     * 修改实体
     * @param  source 修改数据的实体
     * @param  options 修改实体数据
     */
    editAssembleData(source, options) {
        console.log(options)
        //编辑状态实体
        ...
        this.viewer.resource.set(source.id, this.getAssembleJson(source))
    }
    ...
    _changHeight(source,height,isEdit){
        if(isEdit){
            ...
            source.polygon.hierarchy = newPositions.concat(newPositions[0])
