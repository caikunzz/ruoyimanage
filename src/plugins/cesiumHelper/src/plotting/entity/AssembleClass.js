  ...
  createAssemble(options) {
    return new Promise((resolve, reject) => {
      ...
      handler.setInputAction((movement) => {
        ...
        let endCartesian = plottingUtils.getCatesian3FromPX(
          this.viewer,
          endPos
        );
      ...
      handler.setInputAction(() => {
        ...
        if (options?.show instanceof Array) {
          assemblePolygon.polyline.distanceDisplayCondition =
            new Cesium.DistanceDisplayCondition(...options.show);
          assemblePolygon.polygon.distanceDisplayCondition =
            new Cesium.DistanceDisplayCondition(...options.show);
        }
        handler.destroy();
  ...
  showAssemble(options) {
    return new Promise((resolve, reject) => {
      ...
      const degreesArrayHeight = Cesium.Cartesian3.fromDegreesArrayHeights(
        computeAssemblePoints(degreesPositions)
      ).map((item) => {
