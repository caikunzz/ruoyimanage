...
import {
    calculateFormationPoints, lockingMap,
...
class DefenseLineClass{
    ...
    createDefenseLine(options){
        return new Promise((resolve, reject) => {
            ...
            let formationPolyline = undefined;
            ...
            handler.setInputAction((event) => {
                ...
                formationPolyline = this.viewer.entities.add({
                    id: id,
                    name: 'DefenseLine',
                    type:"situation",
                ...
                formationPolyline.GeoType = 'DefenseLine'; //记录对象的类型，用户后续编辑等操作
            ...
            handler.setInputAction((movement) => {
                ...
                const endCartesian = plottingUtils.getCatesian3FromPX(this.viewer, endPos);
            ...
            handler.setInputAction(() => {
                ...
                updateEntityToStaticProperties(formationPolyline,true)
                if (options?.show instanceof Array){
                    formationPolyline.polyline.distanceDisplayCondition = new Cesium.DistanceDisplayCondition(...options.show)
                }
                document.removeEventListener('keydown', cancel)
                document.removeEventListener('keydown', enterEnd)
                resolve(formationPolyline)
            }, Cesium.ScreenSpaceEventType.LEFT_DOUBLE_CLICK);
            const cancel = (e) => {
                if (e.key === 'Escape') {
                    this.viewer.entities.remove(formationPolyline)
                    handler.destroy();
                    toolTips(window.toolTip, null, false);
                    document.removeEventListener('keydown', cancel)
                    document.removeEventListener('keydown', enterEnd)
     ...
     *
     */
    editDefenseLine(viewer, entity, handler, collection){
        ...
        if (editItem) {
            ...
            const newDefenseLine = Cesium.clone(entity.polyline);
            // newDefenseLine.material = Cesium.Color.RED.withAlpha(0.4);
            newDefenseLine.positions = dynamicPositions;
            editEntity = viewer.entities.add({
                GeoType: 'EditDefenseLine',
                Editable: true,
                id: 'edit_' + entity.id,
                polyline: newDefenseLine,
            });
            //编辑状态让实体贴地
        ...
        let currentPickDefenseLine = undefined; //当前选择的要移动的阵型对象
        // 左键摁下事件
        handler.setInputAction((event) => {
            boolDown = true;
            ...
            if (Cesium.defined(pick) && pick.id) {
                ...
                if (pickEntity.GeoType === 'DefenseLineEditPoint') {
                    lockingMap(viewer, false);
                    currentPickVertex = pickEntity;
                } else if (pickEntity.GeoType === 'EditDefenseLine') {
                    lockingMap(viewer, false);
                    currentPickDefenseLine = pickEntity;
                }
            }
        ...
        handler.setInputAction((event) => {
            ...
            if (boolDown && currentPickDefenseLine) { //整体移动
        ...
        handler.setInputAction(() => {
            ...
            currentPickDefenseLine = undefined;
        ...
        function initVertexEntities() {
            ...
            for (let index = 0; index < updatePos.length; index++) {
                ...
                verEntity.GeoType = 'DefenseLineEditPoint';
    ...
    showDefenseLine(options){
        return new Promise((resolve, reject) => {
            ...
            if (entity) {
                ...
                const degreesArrayHeight = calculateFormationPoints(degreesPositions, "").map(item=>{
                ...
                entity = this.viewer.entities.add({
                    ...
                    polyline: {
                        ...
                        width: options.material?.line?.width || 1,
     ...
     */
    editDefenseLineData(source,options){
        //编辑状态实体
        ...
        this.viewer.resource.set(source.id, this.getDefenseLineJson(source))
    }
    /**
     * 导出态势箭头 Json 数据
     *
...
export default DefenseLineClass
