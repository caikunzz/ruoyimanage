...
class FlagClass {
    ...
    createFlag(options) {
        return new Promise((res, rej) => {
            ...
            handler.setInputAction((event) => {
                ...
                const flagCanvas = this.createFlagMaterial(name, options.textColor, options.backgroundColor, options.borderColor)
    ...
    editFlagData(source, options) {
        ...
        if ("camp" in options) {
            ...
            const tc = options.camp === 0 ? "rgba(255, 255, 0, 1)" : options.camp === 1 ? "rgba(255, 255, 255, 1)" : "rgba(0, 255, 255, 1)"
            const bc = options.camp === 0 ? "rgba(255, 0, 0, 1)" : options.camp === 1 ? "rgba(0, 0, 255, 1)" : "rgba(0, 255, 0, 1)"
            ...
            source.billboard.image = this.createFlagMaterial(source.flag.text, tc, fc, bc).src;
        }
        // 修改实体有效期
        ...
        if (options.material) {
            ...
            if (material.border) {
                ...
                if (border.color) {
                    ...
                    source.billboard.image = this.createFlagMaterial(source.flag.text, source.flag.textColor, source.flag.backgroundColor, border.color).src
                }
            }
            // 填充
            if (material.fill) {
                ...
                if (fill.color) {
                    ...
                    source.billboard.image = this.createFlagMaterial(source.flag.text, source.flag.textColor, fill.color, source.flag.borderColor).src
                }
            }
            // 文字
            if (material.text) {
                ...
                source.billboard.image = this.createFlagMaterial(source.flag.text, text.color, source.flag.backgroundColor, source.flag.borderColor).src
            }
            // 航迹线
        ...
        return new Promise((resolve, reject) => {
            ...
            if (entity) {
                ...
                entity = this.viewer.entities.add({
                    ...
                    billboard: {
                        image: this.createFlagMaterial(options.label, options.material.text.color, options.material.fill.color, options.material.border.color).src,
                        pixelOffset: new Cesium.Cartesian2(35, 10),
                        verticalOrigin: Cesium.VerticalOrigin.BOTTOM, // 设置图片垂直方向的原点在底部
                        colorBlendMode: Cesium.ColorBlendMode.REPLACE, // 使用材质的颜色属性替换实体颜色属性,
    ...
    _changName(source, options) {
        const flagCanvas = this.createFlagMaterial(options.name, source.flag.textColor, source.flag.backgroundColor, source.flag.borderColor)
        // 以旗杆底部为原点的偏移
