...
import {
    getRightAngleArrowPoints, lockingMap,
...
class RightAngleArrowClass{
        ...
        return new Promise((resolve, reject) => {
            ...
            handler.setInputAction((move) => {
                ...
                if (!Cesium.defined(rightAngleArrow)) {
                    rightAngleArrow = this.viewer.entities.add({
                        id: id,
                        ...
                        polygon: new Cesium.PolygonGraphics({
                            hierarchy: new Cesium.CallbackProperty(function () {
                                let points = getRightAngleArrowPoints(anchorpoints);
                        ...
                        polyline: {
                            positions: new Cesium.CallbackProperty(function () {
                                const positions = getRightAngleArrowPoints(anchorpoints); // 根据多边形顶点计算多边形形状
            ...
            handler.setInputAction((event) => {
                ...
                updateEntityToStaticProperties(rightAngleArrow)
                if (options?.show instanceof Array){
                    rightAngleArrow.polyline.distanceDisplayCondition = new Cesium.DistanceDisplayCondition(...options.show)
                    rightAngleArrow.polygon.distanceDisplayCondition = new Cesium.DistanceDisplayCondition(...options.show)
                }
                document.removeEventListener('keydown', cancel)
                document.removeEventListener('keydown', enterEnd)
                resolve(rightAngleArrow)
            }, Cesium.ScreenSpaceEventType.LEFT_DOUBLE_CLICK);
            const cancel = (e) => {
                if (e.key === 'Escape') {
                    this.viewer.entities.remove(rightAngleArrow)
                    handler.destroy();
     ...
     *
     */
    editRightAngleArrow(viewer, entity, handler, collection){
        ...
        let lineCallback = new Cesium.CallbackProperty(() => {
                let updatePositions = getRightAngleArrowPoints(updatePos);
                return updatePositions.concat(updatePositions[0])
            }, false);
        if (editItem) {
    ...
    showRightAngleArrow(options) {
        return new Promise((resolve, reject) => {
            ...
            if (existingEntity) {
                ...
                const degreesArrayHeight = getRightAngleArrowPoints(degreesPositions).map(item=>{
     ...
     */
    editRightAngleArrowData(source, options) {
        //编辑状态实体
        ...
        this.viewer.resource.set(source.id, this.getRightAngleArrowJson(source))
    }
...
export default RightAngleArrowClass
