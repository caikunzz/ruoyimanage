...
class RiverPlotting {
      ...
      if (entity) {
        ...
        const availability = dateUtils.iso8602TimesToJulianDate(
          options.availability ||
            dateUtils.julianDateToIso8602Times(this.viewer.clock.currentTime, 5)
        );
        const polylines =
          options.position.polyline?.map((item) => {
            return new Promise((resolve) => {
              const polyline = this.viewer.entities.add({
                ...
                polyline: {
                  positions: item.map((item) =>
                    plottingUtils.latitudeAndLongitudeToDegrees(item)
                  ),
                  material: new Cesium.PolylineGlowMaterialProperty({
                    glowPower: 0.1,
                    color: Cesium.Color.fromCssColorString(
                      options?.material?.fill?.color || "rgba(16,57,255,0.5)"
                    ),
                  }),
        ...
        const polygons =
          options.position.polygon?.map((item) => {
            return new Promise((resolve) => {
  ...
  editRiverPlotting(source, options) {
    ...
    if ("availability" in options) {
      ...
      for (let entity of source.children) {
        ...
        entity.availability = dateUtils.iso8602TimesToJulianDate(
          options.availability
        );
      }
    }
  ...
  getRiverToJson(entity) {
    ...
    return {
      ...
      position: {
        polyline: entity.children
          .filter((children) => children.type.indexOf("polyline") !== -1)
          .map((children) =>
            children.polyline.positions
              .getValue()
              .map((item) => plottingUtils.degreesToLatitudeAndLongitude(item))
          ),
        polygon: entity.children
          .filter((children) => children.type.indexOf("polygon") !== -1)
          .map((children) =>
            children.polygon.hierarchy
