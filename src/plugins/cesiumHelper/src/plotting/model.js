...
class ModelPlotting {
  ...
  createModelPlot(options) {
    return new Promise((resolve) => {
      ...
      if (options?.show instanceof Array) {
        entity.model.distanceDisplayCondition =
  ...
  editModelPlot(source, options) {
    ...
    if ("show" in options) {
      if (options.show instanceof Array) {
        source.show = true;
        source.model.distanceDisplayCondition =
          new Cesium.DistanceDisplayCondition(...options.show);
    ...
    if ("angle" in options) {
      const hpr = new Cesium.HeadingPitchRoll(
        Cesium.Math.toRadians(options.angle),
        0,
        0
      );
      //在实体上设置角度
      ...
      // 计算模型角度
      source.orientation = Cesium.Transforms.headingPitchRollQuaternion(
        source.position.getValue(Cesium.JulianDate.now()),
        hpr
      );
    }
    // 修改样式
  ...
  setPath(entity, options) {
    let times = options.controller
      ? entityRun.generateSpeedTime(options)
      : entityRun.generateTime(options);
  ...
  getModelToJson(entity) {
    ...
    const temp = {
      ...
      endTime: endTime,
      show:
        entity.model.distanceDisplayCondition?.getValue() instanceof Object
          ? [...Object.values(entity.model.distanceDisplayCondition.getValue())]
          : entity.show,
  ...
  _getDefaultName() {
    return (
      "模型" +
      ("000" + (Array.from(this.modelMap.keys()).length + 1)).substr(-3)
    );
  }
}
