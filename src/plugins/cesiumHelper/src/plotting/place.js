...
class PlacePlotting {
    ...
    createPlacePlotting(options) {
        return new Promise((res) => {
            ...
            const availability = dateUtils.iso8602TimesToJulianDate(options?.availability || dateUtils.julianDateToIso8602Times(this.viewer.clock.currentTime))
    ...
    showPlottingForData(options) {
        return new Promise((resolve, reject) => {
            ...
            if (entity) {
                ...
                entity = this.viewer.entities.add({
                    ...
                    point: {
                        ...
                        outlineColor: Cesium.Color.fromCssColorString(options.material.border.color),
                    ...
                    label: {
                        ...
                        pixelOffset: this.getRelativePosition(options.material.text.position),
                        heightReference: Cesium.HeightReference.CLAMP_TO_GROUND,
            ...
            entity.label["data"] = {position: options.material?.text?.position || "top"}
            this.placeMap.set(id, entity)
            this.viewer.resource.set(id, this.getPlaceToJson(entity))
            resolve(entity)
        })
    ...
    editPlacePlotting(source, options) {
        ...
        if ('show' in options){
            if (options.show instanceof Array){
                source.show = true
                source.point.distanceDisplayCondition = new Cesium.DistanceDisplayCondition(...options.show)
    ...
    getPlaceToJson(entity) {
        ...
        const temp = {
            ...
            startTime: startTime,
            endTime: endTime,
            show: entity.point.distanceDisplayCondition?.getValue() instanceof Object ? [...Object.values(entity.point.distanceDisplayCondition.getValue())] : entity.show,
    ...
    copyEntityAdd(entity) {
        return new Promise((resolve, reject) => {
            ...
            pilaceJson.availability = dateUtils.parseDateToString("yyyy-MM-ddTHH:mm:ssZ", new Date(pilaceJson.startTime)) + "/" + dateUtils.parseDateToString("yyyy-MM-ddTHH:mm:ssZ", pilaceJson.endTime)
            this.showPlottingForData(pilaceJson).then(entity => {
                resolve(entity)
            }, error => {
                reject(error)
            })
