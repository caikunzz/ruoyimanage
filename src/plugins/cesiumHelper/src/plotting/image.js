...
class Image {
  ...
  editImagePlot(source, options) {
    ...
    // 修改 实体 角度
    "angle" in options &&
      (source.billboard.rotation = Cesium.Math.toRadians(options.angle));
    // 修改 实体 尺寸
    if ("scale" in options) {
    ...
    if ("material" in options) {
      ...
      if (material.targetLine) {
        ...
        source.material.targetLine = {
          color:
            targetLine.color ||
            source.material.targetLine?.color ||
            "rgba(255,255,255,1)",
          width: targetLine.width || source.material.targetLine?.width || 2,
          style:
            targetLine.style || source.material.targetLine?.style || "solid",
        };
      }
    }
    // 修改动画
    ...
    return new Promise((resolve, reject) => {
      ...
      if (entity) {
        ...
        entity = this.viewer.entities.add({
          ...
          label: {
            text: options.label,
            font: options.material.text.italic
              ? "italic " +
  ...
  getBillboardToJson(entity) {
    ...
    if (entity.label) {
      temp.material.text = {
        ...
        position: this.getRelativePosition({
          show: true,
          offset: entity.label.pixelOffset.getValue(),
        }),
    ...
    if (entity.animation) {
      ...
      if (entity.animation.rotate)
        temp["animation"]["rotate"] = entity.animation.rotate;
      if (entity.animation.scale)
        temp["animation"]["scale"] = entity.animation.scale;
      if (entity.animation.autoRotate)
        temp["animation"]["autoRotate"] = entity.animation.autoRotate;
      if (entity.animation.targetLine)
        temp["animation"]["targetLine"] = entity.animation.targetLine;
  ...
  copyEntityAdd(entity) {
    return new Promise((resolve, reject) => {
      ...
      billboardJson.availability =
        dateUtils.parseDateToString(
          "yyyy-MM-ddTHH:mm:ssZ",
          new Date(billboardJson.startTime)
        ) +
        "/" +
        dateUtils.parseDateToString(
          "yyyy-MM-ddTHH:mm:ssZ",
          billboardJson.endTime
        );
      this.showPlottingForData(billboardJson).then(
        (entity) => {
          resolve(entity);
        },
        (error) => {
  ...
  getRelativePosition(options) {
    ...
    if (options.show) {
      ...
      return [
        positionText[options.textPosition][0],
        positionText[options.textPosition][1],
      ];
    }
  }
  ...
  _getDefaultName() {
    return (
      "图片" +
      ("000" + (Array.from(this.imageMap.keys()).length + 1)).substr(-3)
    );
  }
}
