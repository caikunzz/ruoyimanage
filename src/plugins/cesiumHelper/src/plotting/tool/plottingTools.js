...
export function computeCurveLinePoints(anchorpoints, num) {
  ...
  const lngLatPoints = anchorpoints.map((i) =>
    plottingUtils.degreesToLatitudeAndLongitude(i)
  );
  const pos = bezierSpline(
    lineString(lngLatPoints)
  ).geometry.coordinates.filter((item, i) => i % p === 1);
...
export function updateEntityToStaticProperties(entity, isShowAloneLine) {
  if (
    entity.polygon?.hierarchy instanceof Cesium.CallbackProperty ||
    entity.polyline.positions instanceof Cesium.CallbackProperty
  ) {
    if (isShowAloneLine) {
      ...
      // 获取当前CallbackProperty计算出的多边形和折线值
      const currentPolygonValue = entity.polygon.hierarchy?.getValue(
...
export function getTravelArrowPoints(positions) {
  const lnglats = positions.map((item) =>
    plottingUtils.degreesToLatitudeAndLongitude(item)
  );
  const points =
    lnglats.length > 2
      ? bezierSpline(lineString(lnglats)).geometry.coordinates
      : lnglats;
  // 求最后两点正北方向夹角
  const angle = locationUtils.getAzimuth(
  ...
  const p2 = [
    ...locationUtils.computePointForDegAndLength(p1, angle - 180, arrowLength),
    p1[2],
  ];
  const p3 = [
    ...locationUtils.computePointForDegAndLength(
      p2,
      angle - 180 - 60,
      Math.floor(arrowLength * 0.4)
    ),
    p1[2],
  ];
  const p4 = [
...
export function getRightAngleArrowPoints(anchorpoints) {
  ...
  if (
    degreePoints[0].x === degreePoints[1].x &&
    degreePoints[0].y === degreePoints[1].y &&
    degreePoints[0].z === degreePoints[1].z
  ) {
    return [];
  } //避免两个值相同造成报错
