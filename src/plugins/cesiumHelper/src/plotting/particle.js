...
class Particle {
  ...
  copyEntityAdd(entity) {
    return new Promise((resolve, reject) => {
      ...
      particleJson.availability =
        dateUtils.parseDateToString(
          "yyyy-MM-ddTHH:mm:ssZ",
          new Date(particleJson.startTime)
        ) +
        "/" +
        dateUtils.parseDateToString(
          "yyyy-MM-ddTHH:mm:ssZ",
          particleJson.endTime
        );
      this.showPlottingForData(particleJson).then(
        (entity) => {
          resolve(entity);
        },
        (error) => {
  ...
  getParticlesToJson(entity) {
    ...
    const temp = {
      ...
      position:
        entity.position instanceof Cesium.SampledPositionProperty
          ? this._getPathPoint(entity)
  ...
  showPlottingForData(options) {
    return new Promise((resolve, reject) => {
      this._createGifPlot(options).then(
        (res) => resolve(res),
        () => reject(null)
      );
    });
  ...
  _getDefaultName() {
    return (
      "particle" +
      ("000" + (Array.from(this.particleMap.keys()).length + 1)).substr(-3)
    );
  }
}
