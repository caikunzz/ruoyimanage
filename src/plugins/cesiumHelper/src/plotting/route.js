...
class RoutePlotting {
  ...
  createRoutePlotTest(options) {
    ...
    const curveLine = computeCurvePathPoints(
      options.map((point) => plottingUtils.latitudeAndLongitudeToDegrees(point))
    );
  ...
  createRoutePlot(options) {
    return new Promise((resolve, reject) => {
      ...
      handler.setInputAction(async (event) => {
        ...
        polyline.polyline.positions = heightPoint.map((point) =>
  ...
  getTerrainHeight(point) {
    ...
    const heights = [];
    const PointsLan = point.map((point) =>
    ...
    for (const point of PointsLan) {
      ...
      const pointCartesian = Cesium.Cartesian3.fromRadians(
  ...
  bindRunPath(entity, options) {
    ...
    for (let i = 0; i < options.polyline.length - 1; i++) {
      ...
      const position = Cesium.Cartesian3.fromDegrees(
        options.polyline[i][0],
        options.polyline[i][1],
        options.polyline[i][2]
      );
    ...
    this.routePath.forEach((path) => {
      ...
      if (path.start === path.end) {
        let endIndex = Math.floor(
          (path.end / (this.originalEditPoint.length - 1)) *
            this.originalPoint.length
        );
        endIndex = endIndex > 0 ? endIndex + 1 : 0;
        path.line = [
          this.originalPoint[endIndex],
          this.originalPoint[endIndex],
        ];
      ...
      let startIndex = Math.floor(
        (path.start / (this.originalEditPoint.length - 1)) *
          this.originalPoint.length
      );
      let endIndex = Math.floor(
        (path.end / (this.originalEditPoint.length - 1)) *
  ...
  showPlottingForData(options) {
    return new Promise((resolve, reject) => {
      ...
      if (entity) {
        ...
        const positions = options.polyline.map((position) =>
        ...
        entity = this.viewer.entities.add({
          ...
          EditingPoint: options.memory.editPoint?.map((point) =>
            plottingUtils.latitudeAndLongitudeToDegrees(point)
          ),
        });
        if (options.show instanceof Array)
  ...
  _getPathPoint() {
    let pathData = {
      ...
      type: "draw",
      polyline: [],
      keyPoints: this.originalEditPoint.map((point) =>
        plottingUtils.degreesToLatitudeAndLongitude(point)
      ),
      times: [],
    };
