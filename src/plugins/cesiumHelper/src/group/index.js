...
class Group {
  ...
  removeById(id) {
    ...
    // 此处借助递归删除所有子元素
    const elements = Array.from(this.viewer.resource.values()).filter(
      (p) => p.groupId === id
    );
  ...
  /** 导出编组内容 */
  export(id) {
    const elements = this._findAllChildren(
      Array.from(this.viewer.resource.values()),
      id
    );
  ...
  _computeGroupAvailability(id) {
    ...
    const start = elements.reduce((pre, cur) =>
      new Date(pre.startTime).getTime() > new Date(cur.startTime).getTime()
        ? cur
        : pre
    );
    const end = elements.reduce((pre, cur) =>
  ...
  _changeGroupAvailability(id, availability) {
    ...
    for (const [key, item] of this.viewer.resource) {
      ...
      const startDate = dateUtils.dateDistance(
        new Date(item.startTime),
        distance
      );
      const endDate = dateUtils.dateDistance(new Date(item.endTime), distance);
      let availability =
        dateUtils.parseDateToString("yyyy-MM-ddTHH:mm:ssZ", startDate) +
        "/" +
        dateUtils.parseDateToString("yyyy-MM-ddTHH:mm:ssZ", endDate);
      // 如果类型为摄像头 则只需要结束时间
      if (item.type === "camera")
        availability = dateUtils.parseDateToString(
