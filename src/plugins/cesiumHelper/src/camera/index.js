...
class Camera {
  ...
  updateCamera(currentTime) {
    ...
    setTimeout(() => {
      ...
      if (this.preActiveData) {
        ...
        if (
          Cesium.JulianDate.compare(currentTime, endTime) <= 0 &&
          Cesium.JulianDate.compare(currentTime, startTime) > 0
        ) {
  ...
  _defaultTransition(preCamera, camera, time) {
    ...
    const timeFraction =
      ...
      Cesium.JulianDate.secondsDifference(endTime, startTime);
    ...
    // 插值方向
    const startHPR = new Cesium.HeadingPitchRoll(
      preCamera.angle.head,
      preCamera.angle.pitch,
      preCamera.angle.roll
    );
    const endHPR = new Cesium.HeadingPitchRoll(
      camera.angle.head,
      camera.angle.pitch,
      camera.angle.roll
    );

    const startOrientation = Cesium.Quaternion.fromHeadingPitchRoll(
      startHPR,
      new Cesium.Quaternion()
    );
    const endOrientation = Cesium.Quaternion.fromHeadingPitchRoll(
      endHPR,
      new Cesium.Quaternion()
    );

    // 四元组
    const slerpedOrientation = Cesium.Quaternion.slerp(
      startOrientation,
      endOrientation,
      timeFraction,
      new Cesium.Quaternion()
    ...
    // HeadingPitchRoll标题
    const finallyHeadingPitch =
      Cesium.HeadingPitchRoll.fromQuaternion(slerpedOrientation);
  ...
  _rotatingByPoint(preCamera, camera, time) {
    ...
    const distance = Cesium.Cartesian2.distance(
      Cesium.Cartesian2.fromCartesian3(cameraPosition),
      Cesium.Cartesian2.fromCartesian3(centerPosition)
    );
    ...
    centerPosition = Cesium.Cartesian3.fromDegrees(
      center[0],
      center[1],
      camera.position[2] - tempH
    );
  ...
  _sortSourceList() {
    ...
    cameraList.sort((a, b) => {
      return Cesium.JulianDate.compare(
        Cesium.JulianDate.fromIso8601(a.availability),
        Cesium.JulianDate.fromIso8601(b.availability)
      );
    });
