...
class Video{
    ...
    constructor(viewer, container, supper) {
        ...
        supper.player.addEventListener("play", "playVideo", this.playVideo.bind(this))
        supper.player.addEventListener("pause", "pauseVideo", this.pauseVideo.bind(this))
        supper.player.addEventListener("change", "updateVideo", this.updateVideoState.bind(this))
        supper.player.addEventListener("speed_change", "speed_change_video_callback", this._speedChangeCallback.bind(this))
    }
    ...
    addVideo(options){
        return new Promise((resolve, reject) => {
            try {
                ...
                this._createVideoContainer(options).then(
                    video => {
                        const distance = Math.round(video.duration) && Math.round(video.duration)!==Infinity ? Math.round(video.duration) : 5
                        const temp = {
                            id: options.id,
    ...
    _createVideoGroupContainer(cesiumContainer){
        ...
        style.innerHTML = ".cesium-helper-video{\n" +
            "position: fixed;\n" +
            "z-index: 2;\n" +
