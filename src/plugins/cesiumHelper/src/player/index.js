...
export default class Player {
    ...
    constructor(viewer, clockOptions) {
        ...
        this.clock.onStop.addEventListener(()=>{
            const currentTime = Cesium.JulianDate.toIso8601(viewer.clock.currentTime).substring(0, 23).replace("Z", "") + "Z"
            Array.from(this.callbacks.get("stop").values()).forEach(func => func && func(currentTime))
        })
        this.clock.onTick.addEventListener(clock=>{
            const currentTime = Cesium.JulianDate.toIso8601(clock.currentTime).substring(0, 23).replace("Z", "") + "Z"
            this.isPlaying && Array.from(this.callbacks.get("change").values()).forEach(func => func && func(currentTime))
        })
    }
    ...
    /** 设置播放速度 */
    setSpeed(speed) {
        if (speed !== this.speed) Array.from(this.callbacks.get("speed_change").values()).forEach(item => item(speed))
        this.speed = speed
    ...
    updateTime(time) {
        ...
        Array.from(this.callbacks.get("change").values()).forEach(func => func && func(time))
    }
