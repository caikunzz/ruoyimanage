...
class Infobox {
  ...
  constructor(viewer, cesiumContainer, supper) {
    this.viewer = viewer;
    this.infoboxGroupContainer =
      this._createInfoboxGroupContainer(cesiumContainer);
    ...
    // 添加实体激活默认事件
    supper.addEntityActivateEvent(
      "cesiumHelperInfoboxCreate",
      this.showEntityInfobox.bind(this)
    );
  }
  ...
  _mouseDrag() {
    ...
    this.viewer.scene.preRender.addEventListener(() => {
      for (let [key, value] of this.infoboxMap) {
        ...
        const currentTime = DateUtils.julianDateToDate(
